...
//    [Header("Angles (rotation when spawn VFX)")]
//    public float stabAngle = 0f;        // hit 1 (đâm thẳng)
//    public float slashSideAngle = 0f;   // hit 2 (chém ngang)
...
public class SwordComboMoveSkill : WeaponSkill
{
    ...
    public float hit1Recovery = 0.15f;
    public float hit2Recovery = 0.18f;
    public float hit3Recovery = 0.25f;

    [Header("Movement")]
    public float dashDistance = 1.2f;
    public float dashTime = 0.18f;

    public float jumpUp = 2.8f;
    ...
    [Header("Angles")]
    public float stabAngle = 0f;
    public float slashSideAngle = 0f;
    public float slamAngle = -90f;
    ...
    [Range(0f, 1f)] public float hit1ImpactT = 0.55f;
    [Range(0f, 1f)] public float hit2ImpactT = 0.2f;
    [Range(0f, 1f)] public float hit3ImpactT = 0.85f;
    ...
    IEnumerator ComboStep(SkillContext ctx, State st)
    {
        ...
        if (hit == 0) yield return Hit1(ctx, st, faceRight);
        else if (hit == 1) yield return Hit2(ctx, st, faceRight);
        else yield return Hit3(ctx, st, faceRight);
    ...
    IEnumerator Hit1(SkillContext ctx, State st, bool faceRight)
    {
        if (usePlayerAction && st.action != null)
        {
            float hold = stabLife * 0.3f;
            st.action.PlayHit1_DashStab(faceRight, dashDistance, dashTime, hit1Windup, hold, hit1Recovery);

            yield return new WaitForSeconds(hit1Windup + dashTime * hit1ImpactT);

            Spawn(stabPrefab, ctx, stabOffset, faceRight, stabDamage, stabAtkScale, stabLife, stabAngle);

            yield return new WaitForSeconds(hold + hit1Recovery);
        }
    }

    IEnumerator Hit2(SkillContext ctx, State st, bool faceRight)
    {
        if (usePlayerAction && st.action != null)
        {
            float stepDist = dashDistance * 0.35f;
            ...
            float slashTime = slashSideLife * 0.7f;

            st.action.PlayHit2_Slash(faceRight, stepDist, stepTime, hit2Windup, slashTime, hit2Recovery);

            yield return new WaitForSeconds(hit2Windup + slashSideLife * hit2ImpactT);

            Spawn(slashSidePrefab, ctx, slashSideOffset, faceRight, slashSideDamage, slashSideAtkScale, slashSideLife, slashSideAngle);

            yield return new WaitForSeconds(stepTime + hit2Recovery);
        }
    }

    IEnumerator Hit3(SkillContext ctx, State st, bool faceRight)
    {
        if (usePlayerAction && st.action != null)
        {
            float hold = slamLife * 0.25f;

            st.action.PlayHit3_JumpSlam(faceRight, jumpUp, jumpUpTime, slamDownTime, hit3Windup, hold, hit3Recovery);

            yield return new WaitForSeconds(hit3Windup + jumpUpTime + slamDownTime * hit3ImpactT);

            Vector3 pos = ctx.owner.position + slashDownOffset;

            SpawnAt(pos, ctx, faceRight, slamDamage, slamAtkScale, slamLife, slamAngle, slashDownPrefab);

            yield return new WaitForSeconds(hold + hit3Recovery);
        }
    }

    void Spawn(GameObject prefab, SkillContext ctx, Vector3 offset, bool faceRight,
        int dmg, float scale, float life, float angle)
    {
        float dir = faceRight ? 1f : -1f;
        Vector3 pos = ctx.owner.position + new Vector3(offset.x * dir, offset.y, 0f);

        SpawnAt(pos, ctx, faceRight, dmg, scale, life, angle, prefab);
    }

    void SpawnAt(Vector3 pos, SkillContext ctx, bool faceRight,
        int dmg, float scale, float life, float angle, GameObject prefab)
    {
        float dir = faceRight ? 1f : -1f;
        Quaternion rot = Quaternion.Euler(0, 0, angle * dir);
